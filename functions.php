...
function add_our_script() {
	wp_register_script('accordion-custom-js', plugins_url( 'js/custom.js', __FILE__ ));
	wp_localize_script( 'accordion-custom-js', 'ajax_params', array( 
			'ajax_url' => admin_url( 'admin-ajax.php' ),
			'btn_remove' => plugins_url( 'images/button-remove.png', __FILE__),
			'btn_add' => plugins_url( 'images/add-button.png', __FILE__ ),
...
function scritp_footer() {
	echo '<script src="' . plugins_url( 'js/jquery.easing.1.3.js', __FILE__) . '"></script>';
	echo '<script src="' . plugins_url( 'js/lightbox_accordion.js', __FILE__ ) . '"></script>';
	echo '<script src="' . plugins_url( 'js/jquery.accordion.js', __FILE__ ) . '"></script>';
}
function frontend_accordion_scripts() {
	wp_enqueue_style( 'accordion-style', plugins_url( 'css/style.css', __FILE__ ) );
	wp_enqueue_style( 'lightbox-style', plugins_url( 'css/lightbox.css', __FILE__ ) );
}
